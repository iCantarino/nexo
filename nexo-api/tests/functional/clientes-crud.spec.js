...
test.describe("CRUD Cliente - RF0021 a RF0028 / RN0021 a RN0028", () => {
  const uid = Date.now();
  const emailUnico = "cliente." + uid + "@nexo.com";
  const cpfUnico = String(100+(uid%899)).padStart(3,"0") + "." + String(100+(uid%899)).padStart(3,"0") + "." + String(100+(uid%899)).padStart(3,"0") + "-" + String(10+(uid%89)).padStart(2,"0");
  const payload = { nome: "Cliente Teste Playwright", genero: "Masculino", data_nascimento: "1990-05-15", cpf: cpfUnico, telefone_tipo: "Celular", telefone_ddd: "11", telefone_numero: "987654321", email: emailUnico, senha: "SenhaForte#123" };
  let token = "";
  let idCliente = 0;
  ...
  test("05 login perfil", async ({ request }) => {
    const l = await request.post("/api/v1/auth/cliente/login", { data: { email: payload.email, senha: payload.senha } });
    expect(l.status()).toBe(200);
    token = (await l.json()).access_token;
    const p = await request.get("/api/v1/clientes/me", { headers: { Authorization: "Bearer "+token } });
    expect(p.status()).toBe(200);
    expect((await p.json()).email).toBe(payload.email);
  });
  test("06 admin consulta", async ({ request }) => {
    ...
    const q = await request.get("/api/v1/clientes?email="+payload.email, { headers: { Authorization: "Bearer "+t } });
    expect(q.status()).toBe(200);
    const lista = await q.json();
    expect(lista.length).toBeGreaterThanOrEqual(1);
  });
  test("07 altera perfil", async ({ request }) => {
    const r = await request.patch("/api/v1/clientes/me", { headers: { Authorization: "Bearer "+token }, data: { nome: "Editado PW", telefone_ddd: "21" } });
    expect(r.status()).toBe(200);
    expect((await r.json()).nome).toBe("Editado PW");
  });
  test("08 admin altera", async ({ request }) => {
    const a = await request.post("/api/v1/auth/funcionario/login", { data: { email: "admin@nexo.com", senha: "admin123" } });
    const t = (await a.json()).access_token;
    const r = await request.patch("/api/v1/clientes/"+idCliente, { headers: { Authorization: "Bearer "+t }, data: { nome: "Admin Editou" } });
    expect(r.status()).toBe(200);
  });
  test("09 altera senha", async ({ request }) => {
    ...
    const l = await request.post("/api/v1/auth/cliente/login", { data: { email: payload.email, senha: "NovaForte#456" } });
    expect(l.status()).toBe(200);
  });
